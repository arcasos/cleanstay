...
- DB 스키마 v1 — 15개 테이블, 4단계 SQL 적용
- `region_codes` 서울 493행 적재 (시도 1 + 자치구 25 + 법정동 467)
- `POST /v1/orders` EF 초안 + `_shared/{cors,auth}.ts` (⚠️ 아직 레포에 없음 — 재작성 필요)

**미완료**
- 레포 초기화, 마이그레이션 파일화 (스키마는 DB에 적용됨 → `supabase db pull`로 회수)
- 시드 스크립트(테넌트 + API 키 + 호스트 + 매물)
- `POST /v1/orders` 배포·검증
