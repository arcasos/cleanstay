...
> 이 파일은 매 세션 자동으로 읽힌다. **상세 설계는 `docs/클린콜_프로젝트시드_v5.md`(SSOT)를 참조**하되,
...
| 완료 자동확정 (§13 #4) | `max( completed_at + 6h, deadline_at )` — 상한 없음. 이유는 시드 §13 #4 |
...
우선순위 순. **1번이 지금 작업 중이다.**

0. **`pending_coverage` 일괄 재판정** — 실서비스 전 필수. DB 함수 + cron(1시간).
   지금은 커버리지를 넓혀도 기존 `pending_coverage` 매물이 잠긴 채 남는다.
   `property.activated` webhook 을 보내기로 했는데 **그 이벤트를 발생시킬 주체가 없다.**
   (실제로 `5ea79aa6` 매물이 서초구 개방 후에도 `pending_coverage` 로 굳어 있었다.)

   > **반대 방향(서비스 중단)에 대한 판단**: 매물 `active -> pending_coverage` 는
   > 하되, **기존 발주는 건드리지 않는다.** 커버리지 축소는 "신규 발주를 막는 것"이지
   > "이미 공급자가 수락한 일을 무르는 것"이 아니다. 배차된 발주를 자동 취소하면
   > 공급자 귀책도 아닌 취소가 발생하고 보상 판단이 얽힌다.
   >
   > ⚠️ 함께 볼 구멍: `POST /orders` 는 `property.status` 만 보고 409 를 낸다.
   > 커버리지가 줄었는데 cron 이 아직 안 돌았으면 미지원 지역에 발주가 들어간다.
   > 재판정 cron 을 넣거나, 발주 시 지역 serviceable 을 직접 확인하도록 바꿔야 한다.

1. **공급자 온보딩 + 배차 엔진** ← **지금**
   `POST /v1/providers`, 사업자 진위확인(국세청), 본인인증, 활동지역,
   `GET /v1/offers` + accept/reject, 오퍼 웨이브(§6).
   **`providers = 0` 이라 지금은 어떤 발주도 배차될 수 없다.** 도메인 의존 없음.
2. **호스트 설정 페이지** — 출입정보는 PG 없이 가능(절반). 결제수단은 PG 심사 후.
3. **`confirm` / `dispute` / `retry`** — 아르카 v1 확정 목록(아래).
4. 나머지 webhook.

#### 아르카와 확정한 v1 신규 목록 (2026-07-20, 순서는 위 3~4번)

| 항목 | 비고 |
|---|---|
| `POST /orders/{id}/confirm` | 호스트 확인 → 즉시 `confirmed` |
| `POST /orders/{id}/dispute` | 이의 제기 |
| `POST /orders/{id}/dispute/uploads` | 서명 업로드 URL (15분) |
| `POST /orders/{id}/retry` | 배차 재시도 |
| `GET /hosts/by-ref/{tenant_host_ref}` | 배너 판정용 |
| `order.delayed` / `billing_key.expiring` | webhook 2종 |
| `replaces_order_id` 응답 노출 | 취소→재발주 체인. **새 필드를 만들지 않는다** — 컬럼이 이미 있다 |
| `priority` 서버 판정 | `deadline_at` 까지 24h 미만이면 `urgent` 자동 |

**OpenAPI v0.4 는 1번(공급자 온보딩) 끝나고 정리한다.**
...
**`order.rescheduled` 를 구현하려면 webhook 트리거를 확장해야 한다.**
현재 `enqueue_webhook_delivery()` 는 **`to_status` 만 보고** 이벤트명을 정하고,
`from_status = to_status` 인 이벤트는 아예 건너뛴다(20260720000004).
그 스킵은 필요했다 — `PATCH /orders` 가 동일 상태 이벤트를 쌓는데 트리거가
그걸 `order.dispatched` 로 재발송해, 배차가 다시 일어난 것처럼 보이는
가짜 통지가 나갔다.

문제는 **`order.rescheduled`(`scheduled_at` 변경 통지)도 동일 상태 이벤트**라
같은 스킵에 걸린다는 것이다. 구현하려면 트리거를 "상태가 무엇인가"가 아니라
**"무엇이 바뀌었는가"** 기준으로 바꿔야 한다. `order_events.metadata` 에
변경 필드를 남기고 트리거가 그걸 읽는 형태가 유력하다.

지금은 `PATCH` 로 바뀐 값이 webhook 으로 나가지 않는다.
reconciliation(`GET /orders?updated_since=`)이 잡는다 — `updated_at` 과
`sequence` 가 오르므로 대사에서 드러난다. 백스톱이 원래 그 용도다.
다만 **아르카가 실시간 통지를 기대하면 어긋난다.** 연동 전에 합의가 필요하다.

**`access_update_url` 재조회 시 토큰이 누적된다.**
평문을 저장하지 않으므로 기존 링크를 복원할 수 없어, `GET /orders/{id}` 마다
새 토큰을 발급한다. 만료(마감+7일)로 자연 정리되지만 조회가 잦으면 쌓인다.
...
> **배포는 사람이 한다. Claude 는 코드·검증을 맡는다.**
> `supabase functions deploy` 는 사람이 자기 터미널에서 실행한다.
> CLI 로그인 문제로 네 번 막혔고 그때마다 403 을 PAT 권한 문제로 오진했다.
> Claude 는 배포된 함수에 HTTP 를 쳐서 검증한다 — 여기엔 CLI 권한이 필요 없다.
> 코드가 준비되면 배포를 요청하고, 배포 완료를 확인한 뒤 검증을 돌린다.
...
⚠️ **토큰은 `~/.supabase/access-token` 파일이 아니라 Windows 자격 증명 관리자에 있다.**
`login` 을 다시 해도 구 계정 항목이 남아 덮이지 않는 일이 반복됐다(3회).
계정이 바뀌지 않으면 **먼저 `logout` 하고** 로그인한다:
```
npx supabase logout
npx supabase login --token <PAT>
npx supabase projects list      # cleancall-dev 가 보여야 한다. 안 보이면 실패다
```
`projects list` 에 `A1 (via Readdy)` / `arcasos-dev` 가 보이면 **구 계정**이다.
이 확인 없이 배포를 시도하면 403 이 나고 원인을 PAT 권한으로 오진하게 된다.
