...
**완료** (2026-07-19 기준)
- Supabase 프로젝트 생성 (Seoul)
- DB 스키마 v1 — **21개 테이블**, 4단계 SQL 적용
  (기존 15 + 접근정보 3 `property_access_info`/`order_access_info`/`access_update_tokens`
   + `access_info_views`/`webhook_deliveries`/`order_issues`)
- 마이그레이션 파일화 — `20260718225806_remote_schema.sql`, 원격 이력과 동기화됨
- `region_codes` 서울 493행 적재 + 시드 파일화 (시도 1 + 자치구 25 + 법정동 467)
- 레포 초기화, 랜딩 v1, Tailwind v4 전환

**미완료**
- `_shared/{cors,auth}.ts` — 디렉터리가 비어 있다. **재작성 필요**
- `POST /v1/orders` EF — 레포에 없다. 재작성 → 배포 → 검증
- 시드 스크립트(테넌트 + API 키 + 호스트 + 매물)
- 배차 엔진 EF, webhook EF, 공급자 온보딩 EF
- 프론트 일체(랜딩 제외)

### Supabase 운영 메모
- 프로젝트는 **`cleancall911@gmail.com`** 계정 소유. 사용자의 다른 계정(`jasonjo2011@gmail.com`)으로는
  보이지 않고 link 시 403이 난다. `npx supabase projects list`로 계정부터 확인할 것.
- **`supabase db pull`은 Docker Desktop을 요구하는데 이 개발 머신에 없다.** `pg_dump`로 대체한다:
  ```
  ...
  PG18 pg_dump가 넣는 `\restrict`/`\unrestrict`는 하위 호환을 위해 제거한다.
- CLI 명령에는 항상 `SUPABASE_DB_PASSWORD`를 넘긴다. 빠뜨리면 management API 경로로 빠져
  403(`LegacyDbConfigLoginRoleStatusError`)이 나는데, 계정 문제로 오인하기 쉽다.