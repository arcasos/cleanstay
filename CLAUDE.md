...
테넌트 `arcasos` + API 키 2개 + 호스트 1 + 매물 2 + 커버리지(강남구 `11680` 접두만 개방).

### 6.2 미완료

우선순위 순.

1. **sandbox transition EF** — `POST /sandbox/orders/{id}/transition`
   `ck_test_` 키 전용(live 키면 403 `sandbox_requires_test_key`).
   `to_status` 강제 전이 + `order_events` 기록 + `sequence` 증가.
   `scheduled_at`/`completed_at` 오버라이드(v0.3.1 §2-5) — `free_change_until` 24시간
   경계와 자동확정 시한 검증용. `fault`/`failure_code` 지정 가능.
2. **webhook 발송 EF** — 아르카가 수신 엔드포인트를 만들고 있어 이게 없으면
...
3. 나머지 조회·변경 API (`GET /orders`, `PATCH`, `cancel`, `messages`, `claims`, `coverage`)
4. 배차 엔진 EF (오퍼 웨이브). **원칙 D** — 오퍼를 보내고 공급자가 수락한다
5. 공급자 온보딩 EF
6. 호스트 설정 페이지 (출입정보·결제수단 등록). 현재 토큰만 발급되고 페이지가 없다
7. 프론트 일체(랜딩 제외)

### 6.3 미해결 이슈

**`hosts`·`billing_keys`에 `env` 컬럼이 없다 — 결정 필요.**
`orders`/`properties`는 env로 분리되는데 `hosts`는 아니라서, test 발주가 live 호스트에
붙는다. 그 호스트의 live `billing_keys`가 보이므로 **sandbox 발주가 실제 결제수단을 근거로
`billing_verified`가 될 수 있다.** 반대로 test 요청의 `phone`/`email`이 live가 쓰는 호스트
행에 기록된다. 실증했다(test 매물이 live 호스트 id `0e49b389…`에 연결됨).
`hosts.env` + `billing_keys.env` 추가 여부는 사람에게 물어야 한다.

**`SETUP_BASE_URL` 기본값이 `https://setup.cleancall.local`이다.** §13 #1 도메인 미정.
발급된 링크가 실제로 열리지 않는다. 도메인 확정 시 EF 시크릿에 넣는다.

**dev DB에 검증 잔여물이 있다.** 발주 4건 + `order_events` 5건.
...
- **`env` 필터를 빠뜨리지 않았는지 매번 확인한다.** 세 번 연속 나왔다:
  ...
  새 쿼리를 쓸 때마다 `env` 컬럼이 있는 테이블인지 먼저 본다.
